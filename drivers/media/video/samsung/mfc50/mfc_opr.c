...
void      mfc_context_info(mfc_inst_ctx *mfc_ctx);
#endif
//===========================================================================================================
...
int mfc_load_firmware()
{
	...
	memcpy((void *)fw_virbuf, mfc_fw_code, mfc_fw_code_len);
        ...
        mfc_mem_cache_clean(fw_virbuf, mfc_fw_code_len);
#endif
...
MFC_ERROR_CODE mfc_init_encode(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
   ...
   if(mCheckType == false)
   {
		...
		if(nSize > BOUND_MEMORY_SIZE)
             {
             	mfc_err("mfc_init_encode : MFCINST_ERR_FRM_BUF_SIZE....nSize(%d) \n",nSize);
   ...
   else
   {
	...
	mfc_write_shared_mem(mfc_ctx->shared_mem_paddr, mfc_ctx->shared_mem_vaddr, &(mfc_ctx->shared_mem));

	ret_code = mfc_encode_header(mfc_ctx, args);

	mfc_read_shared_mem(mfc_ctx->shared_mem_paddr, mfc_ctx->shared_mem_vaddr, &(mfc_ctx->shared_mem));
		...
		if(nSize > BOUND_MEMORY_SIZE)
		{
			...
			if (mfc_ctx->InstNo >= 0)
				mfc_return_inst_no(mfc_ctx->InstNo, mfc_ctx->MfcCodecType);
                        ...
                        mfc_err("mfc_init_decode : MFCINST_ERR_FRM_BUF_SIZE....nSize(%d) \n",nSize);
			return MFCINST_ERR_FRM_BUF_SIZE;
		}
   }
   else
...
//             unsigned char *stream_vir;
//		stream_vir = phys_to_virt(dec_arg->in_strm_buf);
//             dma_unmap_single(NULL, (void *)stream_vir, dec_arg->in_strm_size, DMA_FROM_DEVICE);
...
static MFC_ERROR_CODE mfc_alloc_context_buffer(mfc_inst_ctx *mfc_ctx, unsigned int mapped_addr, unsigned int *context_addr, int *size)
{
	...
	dmac_map_area(context_vir, local_param.mem_alloc.buff_size , DMA_TO_DEVICE);
	outer_clean_range(local_param.mem_alloc.out_paddr, local_param.mem_alloc.out_paddr + local_param.mem_alloc.buff_size );
#endif
...
//  dma_unmap_single(NULL, data, nSize, DMA_FROM_DEVICE);
...
//  dma_map_single(NULL, data, nSize, DMA_TO_DEVICE);
...
void    mfc_context_info(mfc_inst_ctx *mfc_ctx)
{
	mfc_err("============== MFC Context Info  ================= \n");
        mfc_err("InstNo(%d), DPBCnt(%d), totalDPBCnt(%d), extraDPB(%d)\n", mfc_ctx->InstNo, mfc_ctx->DPBCnt, mfc_ctx->totalDPBCnt, mfc_ctx->extraDPB);
        mfc_err("displayDelay(%d), postEnable(%d), endOfFrame(%d), forceSetFrameType(%d)\n", mfc_ctx->displayDelay, mfc_ctx->postEnable, mfc_ctx->endOfFrame, mfc_ctx->forceSetFrameType);
        mfc_err("img_width(%d), img_height(%d), dwAccess(%d), IsPackedPB(%d)\n", mfc_ctx->img_width, mfc_ctx->img_height, mfc_ctx->dwAccess, mfc_ctx->IsPackedPB);
        mfc_err("interlace_mode(%d), sliceEnable(%d), crcEnable(%d), widthDivX311(%d)\n", mfc_ctx->interlace_mode, mfc_ctx->sliceEnable, mfc_ctx->crcEnable, mfc_ctx->widthDivX311);
        mfc_err("heightDivX311(%d), mem_inst_no(%d), FrameType(%d), MfcCodecType(%d)\n", mfc_ctx->heightDivX311, mfc_ctx->mem_inst_no, mfc_ctx->FrameType, mfc_ctx->MfcCodecType);
        mfc_err("MfcState(%d), port0_mmap_size(%d), codec_buff_paddr(%d), pred_buff_paddr(%d)\n", mfc_ctx->MfcState, mfc_ctx->port0_mmap_size, mfc_ctx->codec_buff_paddr, mfc_ctx->pred_buff_paddr);
        mfc_err("dec_dpb_buff_paddr(%d), shared_mem_paddr(%d), shared_mem_vaddr(%d), IsStartedIFrame(%d)\n", mfc_ctx->dec_dpb_buff_paddr, mfc_ctx->shared_mem_paddr, mfc_ctx->shared_mem_vaddr, mfc_ctx->IsStartedIFrame);
}
