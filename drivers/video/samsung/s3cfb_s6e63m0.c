...
static int s6e63m0_spi_write(struct s6e63m0 *lcd,
...
static int s6e63m0_gamma_ctl(struct s6e63m0 *lcd)
{
	...
	struct s6e63m0_panel_data *pdata = lcd->lcd_pd->pdata;
	...
	if (lcd->acl_enable) {
		if (lcd->cur_acl == 0) {
			if (lcd->bl == 0 || lcd->bl == 1) {
				ret = s6e63m0_panel_send_sequence(lcd, pdata->acl_table[0]);
				dev_dbg(lcd->dev, "ACL_cutoff_set Percentage : off!!\n");
			} else
				ret = s6e63m0_panel_send_sequence(lcd, pdata->acl_on);
		}
		switch (lcd->bl) {
		case 0 ... 1: /* 30cd ~ 40cd */
			if (lcd->cur_acl != 0) {
		...
		case 2 ... 12: /* 70cd ~ 180cd */
			if (lcd->cur_acl != 40) {
				ret = s6e63m0_panel_send_sequence(lcd, pdata->acl_table[1]);
		...
		case 13: /* 190cd */
			if (lcd->cur_acl != 43) {
				ret = s6e63m0_panel_send_sequence(lcd, pdata->acl_table[2]);
		...
		case 14: /* 200cd */
			if (lcd->cur_acl != 45) {
				ret = s6e63m0_panel_send_sequence(lcd, pdata->acl_table[3]);
		...
		case 15: /* 210cd */
			if (lcd->cur_acl != 47) {
				ret = s6e63m0_panel_send_sequence(lcd, pdata->acl_table[4]);
		...
		case 16: /* 220cd */
			if (lcd->cur_acl != 48) {
				ret = s6e63m0_panel_send_sequence(lcd, pdata->acl_table[5]);
		...
		default:
			if (lcd->cur_acl != 50) {
				ret = s6e63m0_panel_send_sequence(lcd, pdata->acl_table[6]);
...
static int s6e63m0_ldi_init(struct s6e63m0 *lcd)
{
	...
	const unsigned short *init_seq[] = {
		pdata->seq_panel_condition_set,
		pdata->seq_display_set,
		pdata->default_gamma,
		pdata->seq_etc_set,
	};
...
static int s6e63m0_ldi_enable(struct s6e63m0 *lcd)
{
	...
	ret = s6e63m0_panel_send_sequence(lcd, pdata->display_on);
...
static int s6e63m0_ldi_disable(struct s6e63m0 *lcd)
{
	...
	lcd->ldi_enable = 0;
	ret = s6e63m0_panel_send_sequence(lcd, pdata->display_off);
...
static int s6e63m0_set_brightness(struct backlight_device *bd)
{
	...
	if ((lcd->ldi_enable) && (lcd->current_brightness != lcd->bl)) {
		...
		if (ret < 0) {
			/*
			dev_err(&bd->dev, "skip update brightness. because s6e63m0 is on suspend state...\n");
			*/
			return -EINVAL;
		}
	}
...
#ifdef CONFIG_HAS_EARLYSUSPEND
void s6e63m0_early_suspend(struct early_suspend *h)
{
	struct s6e63m0 *lcd = container_of(h, struct s6e63m0,
								early_suspend);
	dev_info(lcd->dev, "+%s\n", __func__);
...
void s6e63m0_late_resume(struct early_suspend *h)
{
	struct s6e63m0 *lcd = container_of(h, struct s6e63m0,
								early_suspend);
	dev_info(lcd->dev, "+%s\n", __func__);
...
static int s6e63m0_probe(struct spi_device *spi)
{
	int ret = 0;
	struct s6e63m0 *lcd;
	struct s6e63m0_panel_data *pdata;
	...
	lcd->ld = lcd_device_register("s6e63m0", &spi->dev,
		lcd, &s6e63m0_lcd_ops);
	if (IS_ERR(lcd->ld)) {
		ret = PTR_ERR(lcd->ld);
		goto out_free_lcd;
