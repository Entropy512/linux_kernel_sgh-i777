	...
	case CY_RQT_TURBO_CMD_FROM_HOST:
		{
			...
			//skkm
			if( ep_p->queue_p == NULL )
			{
				cy_as_hal_print_message("[%s]no queue : ep=%d , mtp_datalen=%d \n", __func__, ep_p->ep, mtp_datalen) ;
				cy_as_dma_cancel(dev_p, CY_AS_MTP_READ_ENDPOINT, CY_AS_ERROR_INVALID_REQUEST);
				break;
			}
