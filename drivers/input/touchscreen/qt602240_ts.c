...
#define CONFIG_PM 1
static struct i2c_client *client_copy;
...
static const u8 init_vals_ver_22[] = {
	...
	0x0a, 0x00, 0x05, 0x00, 0x00, 0x00, 0x09, 0x23,
	/* QT602240_TOUCH_MULTI(9) */
	0x00, 0x00, 0x00, 0x13, 0x0b, 0x00, 0x00, 0x20, 0x02, 0x00,
	0x00, 0x01, 0x01, 0x0e, 0x0a, 0x0a, 0x0a, 0x0a, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0xeb, 0x8f, 0x28, 0x8f, 0x50,
	0x00,
	/* QT602240_TOUCH_KEYARRAY(15) */
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00,
	...
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	/* QT602240_SPT_CTECONFIG(28) */
	0x00, 0x00, 0x00, 0x08, 0x10, 0x0a,
};//CTECONFIG_BYTE 5 init value USED TO BE 0x00
...
////////////Added by jerry///////////
static char *qt602249_object_title_table(unsigned int type)
{
  char *msg;
  	switch (type) {
  	case QT602240_GEN_MESSAGE:      msg="MESSAGE";     break;
	case QT602240_GEN_COMMAND:      msg="COMMAND";     break;
	case QT602240_GEN_POWER:        msg="POWER";       break;
	case QT602240_GEN_ACQUIRE:      msg="ACQUIRE";     break;
	case QT602240_TOUCH_MULTI:      msg="MULTI";       break;
	case QT602240_TOUCH_KEYARRAY:   msg="KEYARRAY";    break;
	case QT602240_TOUCH_PROXIMITY:  msg="PROXIMITY";   break;
	case QT602240_PROCI_GRIPFACE:   msg="GRIPFACE";    break;
	case QT602240_PROCG_NOISE:      msg="NOISE";       break;
	case QT602240_PROCI_ONETOUCH:   msg="ONETOUCH";    break;
	case QT602240_PROCI_TWOTOUCH:   msg="TWOTOUCH";    break;
	case QT602240_SPT_COMMSCONFIG:  msg="COMMSCONFIG"; break;
	case QT602240_SPT_GPIOPWM:      msg="GPIOPWM";     break;
	case QT602240_SPT_SELFTEST:     msg="SELFTEST";    break;
	case QT602240_SPT_CTECONFIG:    msg="CTECONFIG";   break;
	case QT602240_SPT_USERDATA:     msg="USERDATA";    break;
	default:                        msg="No match";    break;
  	}
	return msg;
...
static int qt602240_initialize(struct qt602240_data *data)
{
	...
	u8 val;
    ///////////added by jerry////////
    printk("qt602240_initialize() is called\n");
	/////////////////////////////////////////////
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	int error;
	u8 val;
        printk("qt602240_object_show() is called\n");

	for (i = 0; i < data->info.object_num; i++) {
		object = data->object_table + i;

        count += sprintf(buf + count,
				"Object Element %d(T%d), -->%s\n",
				i + 1, object->type,qt602249_object_title_table(object->type));
...
static int qt602240_load_fw(struct device *dev, const char *fn)
{
	...
	if (ret) {
		...
		printk("Unable to open firmware %s\n", fn);
		return ret;
	}
					...
					const char *buf, size_t count)
{
  ...
  qt602240_write_object(data, (u8)object_type,
			(u8)object_register,
			(u8)register_value);
  qt602240_read_object(data, (u8)object_type,
			(u8)object_register,
			&val);
...
///////////////Added by jerry////////////////////////////////////////
static DEVICE_ATTR(object_show, 0664, qt602240_object_show, NULL);
...
{
	struct qt602240_data *data = i2c_get_clientdata(client_copy);
	struct input_dev *input_dev = data->input_dev;
	...
	disable_irq(client_copy->irq);

	return 0;
	printk("qt602240_early_suspend success\n");
}

static int qt602240_early_resume(struct early_suspend *h)
{
	struct qt602240_data *data = i2c_get_clientdata(client_copy);
	struct input_dev *input_dev = data->input_dev;

	set_irq_type(client_copy->irq, IRQ_TYPE_EDGE_FALLING);
	enable_irq(client_copy->irq);
		...
		const struct i2c_device_id *id)
{
	...
	touch_early_suspend.suspend = (void *) qt602240_early_suspend;
	touch_early_suspend.resume = (void *) qt602240_early_resume;
	register_early_suspend(&touch_early_suspend);
