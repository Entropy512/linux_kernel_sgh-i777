...
#define BUFFER_DATA_SIZE			1600	/* maximum allocated data size,  mtu 1400 so 3 blocks max 1536 */
...
{\
    *pLen = 0;\
    *pLen = sdio_readw(Adapter->func, (SDIO_RX_BANK_ADDR + (*pRIdx * SDIO_BANK_SIZE)) , Status);\
    if (*Status == 0)\
    *pLen |= sdio_readw(Adapter->func, (SDIO_RX_BANK_ADDR + (*pRIdx * SDIO_BANK_SIZE) + 2), Status) << 16;\
...
{\
    *pWIdx = sdio_readb(Adapter->func, SDIO_H2C_WP_REG, Status);\
    if(*Status == 0)\
    if ( ( (*pWIdx + 1) % 15) == sdio_readb(Adapter->func, SDIO_H2C_RP_REG, Status) )\
		*pWIdx = -1;\
}
