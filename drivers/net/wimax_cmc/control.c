...
void control_remove(struct net_adapter *adapter)
{
	...
	while (!queue_empty(adapter->ctl.q_received.head)) {
		...
		dsc = (struct buffer_descriptor *)queue_get_head(adapter->ctl.q_received.head);
		if (!dsc) {
					...
					u_short type, void *buffer, u_long length)
{
	...
	if (process) {
		dsc = (struct buffer_descriptor *)kmalloc(sizeof(struct buffer_descriptor), GFP_ATOMIC);
		if (dsc == NULL) {
...
void remove_process(struct net_adapter *adapter, u_int id)
{
	...
	curElem = (struct process_descriptor *)queue_get_head(adapter->ctl.apps.process_list);

	for ( ; curElem != NULL; prevElem = curElem, curElem  = (struct process_descriptor *)curElem->node.next) {
		if (curElem->id == id) {	/* process found */
			if (prevElem == NULL) {
				/* only one node present */
				if (!((adapter->ctl.apps.process_list).next = ((struct list_head *)curElem)->next)) {
				...
				(adapter->ctl.apps.process_list).prev = (struct list_head *)(prevElem);
			} else {
				/* middle node */
