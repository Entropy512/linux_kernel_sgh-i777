...
test_done:
	...
	// change@wtl.kSingh - FIPS mode self test failure error - starts
	{
		...
		printk(KERN_INFO "%s: %s alg self test failed in fips mode!\n",
		       driver, alg);
...
// change@wtl.kSingh - we allow non-FIPS algorithms - starts
	if (!rc)
		printk(KERN_INFO "alg: self-tests for non-FIPS %s (%s) passed\n",
		       driver, alg);
	else
		printk(KERN_INFO "alg: self-tests for non-FIPS %s (%s) failed\n", alg, driver);
	return rc;
